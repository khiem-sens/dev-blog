...
const Home: NextPage = ({
  ...
  const filerLabel = (tag: any, index: number) => {
      ...
      <section className="flex flex-col items-center text-[1.15rem] mt-12">
        <div className="flex gap-3 mb-12">
          {tags.map((tag: string, index: number) => {
            return (
              <button
                className={`${selectedIdx.includes(index) ? 'label-selected hover:bg-sky-400 transition-all duration-300' : 'label hover:bg-sky-400 transition-all duration-300'}`}
                key={index}
